...
=============================================================================
ML Model: Random Forest with 8 features (95.94% accuracy)
Features: Voltage, Current, Temperature, Light_Intensity, Humidity, 
          ThermistorTemp, RelayStatus, Efficiency
Predictions: 5 Fault Classes (Normal, Partial_Shading, Dust_Accumulation, 
            Open_Circuit, Short_Circuit)
Features:
- Real-time ML-powered fault detection
- WhatsApp notifications via pywhatkit (uses WhatsApp Web)
- Simulator mode with realistic fault injection
- Serial and WiFi ESP32 support
- CSV data logging with automatic retry logic
=============================================================================
"""
...
class SensorData(BaseModel):
    ...
    humidity: float = 50.0          # Default: typical dry ambient
    thermistor_temp: float = 25.0   # Default: room temperature
    relay_status: int = 0            # 0=not triggered, 1=triggered
...
# =============================================================================
# SIMULATOR - 5 FAULT PROFILES (with all 8 features)
# =============================================================================
FAULT_PROFILES = {
    0: {  # Normal
        "name": "Normal", 
        "voltage": (17, 22), "current": (4, 6), "temp": (25, 45), "light": (800, 1200), 
        "humidity": (30, 60), "thermistor": (35, 50), "relay_prob": 0.05,
        "efficiency": (15, 22)
    },
    1: {  # Open_Circuit
        "name": "Open_Circuit", 
        "voltage": (20, 25), "current": (0, 0.15), "temp": (25, 40), "light": (700, 1200), 
        "humidity": (35, 65), "thermistor": (25, 40), "relay_prob": 0.60,
        "efficiency": (0, 2)
    },
    2: {  # Partial_Shading
        "name": "Partial_Shading", 
        "voltage": (8, 15), "current": (1, 3.5), "temp": (30, 55), "light": (150, 450), 
        "humidity": (40, 70), "thermistor": (30, 45), "relay_prob": 0.20,
        "efficiency": (5, 12)
    },
    3: {  # Short_Circuit
        "name": "Short_Circuit", 
        "voltage": (0, 4), "current": (6, 10), "temp": (55, 85), "light": (500, 1200), 
        "humidity": (60, 85), "thermistor": (70, 95), "relay_prob": 0.95,
        "efficiency": (0, 3)
    },
    4: {  # Dust_Accumulation
        "name": "Dust_Accumulation", 
        "voltage": (14, 19), "current": (3, 5), "temp": (35, 55), "light": (400, 700), 
        "humidity": (20, 50), "thermistor": (50, 70), "relay_prob": 0.25,
        "efficiency": (10, 16)
    },
}
...
def generate_simulated_data(fault_type: int = 0) -> dict:
    ...
    humidity = random.uniform(*profile["humidity"]) + random.gauss(0, 2)
    thermistor = random.uniform(*profile["thermistor"]) + random.gauss(0, 1)
    ...
    relay_status = 1 if random.uniform(0, 1) < profile["relay_prob"] else 0
    ...
    return {
        ...
        "thermistor_temp": round(thermistor, 2),
...
def predict_fault(data: SensorData) -> PredictionResponse:
    ...
    # Prepare features (8 features: V, I, T, Light, Humidity, Thermistor, Relay, Efficiency)
    features = np.array([[
        data.voltage, 
        data.current, 
        data.temperature, 
